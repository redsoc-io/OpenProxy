...
function Stats() {
  ...
  useEffect(() => {
    fetch("https://api.github.com/repos/midhunvnadh/OpenProxy")
      .then((res) => res.json())
      .then((json) => {
        setStats(json);
      });
  ...
  return (
    <div className="w-full lg:w-4/6 bg-gray-200 rounded-md flex-wrap shadow-lg overflow-hidden border flex items-center">
      <div className="w-full lg:w-2/6 h-full">
        <div className="flex items-center justify-center p-3">
          ...
          >
            ...
            <span className="font-black text-xl">Open Proxy Project</span>
      ...
      <div className="flex w-full lg:w-4/6">
        <StatsUnit
          name="Watchers"
          text={stats?.subscribers_count || 0}
          icon={<AiOutlineEye />}
        />
        <StatsUnit
          name="Stars"
          text={stats?.stargazers_count || 0}
          icon={<AiOutlineStar />}
        />
        <StatsUnit
          name="Forks"
...
function AboutMd({ readme }) {
  return (
    <div className="markdown-body p-12">
      <ReactMarkdown linkTarget={"_blank"}>{readme}</ReactMarkdown>
    </div>
  );
}
...
export const getServerSideProps = async () => {
  const res = await fetch(
    "https://raw.githubusercontent.com/midhunvnadh/OpenProxy/main/README.md"
  );
